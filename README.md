...
添加持仓时，股票、ETF 和可转债使用市值、单位成本价（支持四位小数）、持仓数量和现价录入，盈亏按现价自动计算；基金可以手工填写当前持仓，也可以导入 PDF 交易流水，并在写入数据前勾选要保留的基金。
PDF 导入后的基金可直接从持仓行进入费率设置。持仓清零时可归档完整交易流水与最终盈亏，并在“已归档”中按股票类、基金分类查看和填写交易心得。总览和已归档页都提供带二次确认的批量删除。
